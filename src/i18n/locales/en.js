export default {
  app: {
    name: 'Quran Player',
    somethingWrong: 'Something went wrong',
    reload: 'Reload App',
    offline: 'You are offline. Some features may not be available.'
  },
  header: {
    settings: 'Settings',
    quickSettings: 'Quick Settings',
    search: 'Search',
    theme: 'Theme',
    autoHide: 'Auto-hide',
    verses: 'Verses',
    showBookmarks: 'Show bookmarks',
    showShortcuts: 'Show keyboard shortcuts',
    toggleQuickSettings: 'Toggle quick settings bar',
    changeTheme: 'Change theme',
    toggleAutoHide: 'Toggle auto-hide controls',
    showVerses: 'Show verse list',
    selectTheme: 'Select {name} theme',
    auto: 'Auto',
    display: 'Display',
    toggleDisplay: 'Toggle display settings',
    collapseBar: 'Collapse settings bar',
    arabicSize: 'Arabic Size',
    translationSize: 'Translation Size',
    contentWidth: 'Content Width',
    jumpToJuz: 'Jump to juz',
    selectJuz: 'Select juz',
    more: 'More',
    listenMode: 'Listen',
    readMode: 'Read',
    layoutSingle: 'Single',
    layoutContinuous: 'Continuous',
    listenModeShort: 'Listen',
    readModeShort: 'Read',
    layoutSingleShort: 'Single',
    layoutContinuousShort: 'Scroll',
    listenModeDesc: 'Play audio with the full player',
    readModeDesc: 'Quiet reading with a compact bar',
    layoutSingleDesc: 'Focus on one verse at a time',
    layoutContinuousDesc: 'Scroll the whole surah',
    activityMode: 'How you use it',
    layoutMode: 'How verses appear',
    modeMenuTitle: 'Reading mode',
    modeMenu: 'Mode: {summary}',
    modeMenuHint: 'Also available under Settings · Reading'
  },
  controls: {
    play: 'Play',
    pause: 'Pause',
    loadingAudio: 'Loading audio',
    seekingAudio: 'Seeking audio',
    repeat: 'Repeat',
    repeatVerse: 'Verse',
    repeatSurah: 'Surah',
    cycleRepeat: 'Cycle repeat mode',
    prevSurah: 'Prev Surah',
    previousSurah: 'Previous surah',
    prev: 'Prev',
    previousVerse: 'Previous verse',
    next: 'Next',
    nextVerse: 'Next verse',
    nextSurah: 'Next Surah',
    playbackSpeed: 'Playback speed',
    audioTools: 'Audio tools',
    volume: 'Volume',
    repeatRange: 'Repeat range',
    setA: 'Set A',
    setB: 'Set B',
    clear: 'Clear',
    sleepTimer: 'Sleep timer',
    audioUnavailable: 'Audio is not available for this surah',
    playFailed: 'Playback failed. Check your connection and try again.',
    retryPlay: 'Retry',
    dismissPlayFailed: 'Dismiss',
    verseOf: 'Verse {current} of {total}',
    jumpToVerse: 'Jump to verse',
    goTo: 'Go to',
    go: 'Go',
    seek: 'Seek audio',
    seekValueOf: '{current} of {total}',
    noAudioLoaded: 'No audio loaded'
  },
  verse: {
    actions: 'Verse actions',
    bookmark: 'Bookmark this verse',
    removeBookmark: 'Remove bookmark',
    share: 'Share this verse',
    copy: 'Copy verse text',
    copied: 'Copied',
    linkCopied: 'Link copied',
    openTafsir: 'Open tafsir',
    footnoteN: 'Footnote {n}',
    footnoteTitle: 'Footnote {n}',
    selectFootnote: 'Show footnote {n}',
    loadingFootnote: 'Loading footnote...',
    footnoteError: 'Could not load footnote. Check your connection.',
    closeFootnote: 'Close footnote',
    retry: 'Retry',
    retryAria: 'Retry loading surah',
    endOf: 'End of {surah}'
  },
  panels: {
    allVerses: 'Verse list',
    closeVerses: 'Close verse list',
    bookmarks: 'Bookmarks',
    clearAll: 'Clear all',
    clearAllConfirm: 'Confirm clear all',
    bookmarksCleared: 'All bookmarks cleared',
    noBookmarks: 'No bookmarks yet',
    noBookmarksHint: 'Tap the bookmark icon on any verse to save it here',
    removeBookmarkItem: 'Remove bookmark for {surah} verse {verse}',
    close: 'Close',
    searchPlaceholder: 'Search surahs or verses in this surah',
    searchQuery: 'Search query',
    closeSearch: 'Close search',
    clearSearch: 'Clear search',
    surahs: 'Surahs',
    recent: 'Recent',
    suggested: 'Suggested',
    suggestedHint: 'Popular surahs to start with',
    versesIn: 'Verses in {surah}',
    noMatchesTitle: 'No matches',
    noMatches: 'Try a surah name or number. Verse search covers the current surah only.',
    tafsir: 'Tafsir',
    tafsirSource: 'Tafsir source',
    closeTafsir: 'Close tafsir',
    loadingTafsir: 'Loading tafsir...',
    tafsirError: 'Could not load tafsir. Check your connection.',
    footnotes: 'Footnotes',
    footnoteList: 'Footnotes on this verse',
    footnoteOf: 'Note {current} of {total}',
    prevFootnote: 'Previous footnote',
    nextFootnote: 'Next footnote',
    dismissHint: 'Tap outside, the same note again, or press Esc to close',
    shortcuts: 'Keyboard Shortcuts',
    closeShortcuts: 'Close shortcuts',
    shortcutPlay: 'Play / Pause',
    shortcutPrev: 'Previous verse',
    shortcutNext: 'Next verse',
    shortcutHelp: 'Toggle this help'
  },
  settings: {
    title: 'Settings',
    close: 'Close settings',
    tabPlayback: 'Playback',
    tabDisplay: 'Display',
    tabReading: 'Reading',
    tabApp: 'App',
    surah: 'Surah',
    reciter: 'Reciter',
    translationLanguage: 'Translation Language',
    translation: 'Translation',
    additionalTranslations: 'Additional translations',
    addTranslation: 'Add a translation...',
    searchSurah: 'Search surah...',
    searchReciter: 'Search reciter...',
    searchTranslation: 'Search translation...',
    searchFont: 'Search font...',
    arabicFont: 'Arabic Font',
    mushafFontOption: 'Mushaf (QCF)',
    arabicFontSize: 'Arabic Font Size',
    translationFontSize: 'Translation Font Size',
    contentWidth: 'Content Width',
    theme: 'Theme',
    playbackSpeed: 'Playback Speed',
    repeatMode: 'Repeat Mode',
    uiLanguage: 'App Language',
    autoHideDuringPlayback: 'Auto-hide controls during playback',
    wordHighlight: 'Word-by-word highlighting',
    wordHighlightHintVerse: 'Only available with reciters that support full surah audio',
    wordHighlightHint: 'Highlights each word as it is recited',
    highlightStyle: 'Highlight Style',
    animations: 'Animations',
    animationsHint: 'Enable transitions and animations',
    readMode: 'Read mode',
    readModeHint:
      'Focus on reading: compact player and select verses without auto-play (works with either layout)',
    readingLayout: 'Reading layout',
    readingLayoutHint: 'How verses are shown on the page',
    layoutSingle: 'Single verse',
    layoutContinuous: 'Continuous',
    continuousReading: 'Continuous reading',
    continuousReadingHint: 'Show all verses in a scrollable page; tap a verse to play',
    footnotes: 'Translation footnotes',
    footnotesHint: 'Show clickable note markers in the translation when available',
    footnotesBadge: 'Notes',
    footnotesAvailableHint:
      'This edition includes footnotes. Tap numbered markers in the translation to read them.',
    tajweed: 'Tajweed colors',
    tajweedHint: 'Color-code letters by pronunciation rule',
    tajweedUnavailableQcf: 'Not available with the mushaf (QCF) font',
    justifyUnavailableQcf: 'The mushaf (QCF) font already spaces words for the page',
    endOrnamentUnavailableQcf: 'The mushaf (QCF) font shows its own ayah numbers',
    justify: 'Justify Arabic text',
    justifyHint: 'Spread long verses to both edges (mushaf style)',
    verseButtons: 'Verse buttons',
    verseButtonsHint: 'Choose which actions appear under each verse',
    endOrnament: 'End-of-verse ornament',
    endOrnamentHint: 'Show the ayah number inline instead of the badge',
    download: 'Download this surah for offline',
    downloading: 'Downloading {surah}...',
    downloadingProgress: 'Downloading {surah}: {current}/{total}',
    downloadCancel: 'Cancel download',
    downloaded: 'Saved offline (tap to remove)',
    downloadFailed: 'Download failed. Check your connection.',
    surahsSaved: '{count} surah(s) saved',
    install: 'Install App',
    version: 'Version {version}',
    actionBookmark: 'Bookmark',
    actionShare: 'Share',
    actionCopy: 'Copy',
    actionTafsir: 'Tafsir',
    checkUpdates: 'Check for Updates',
    checking: 'Checking...',
    upToDate: 'Already up to date',
    updateFound: 'Update found! Reloading...',
    reset: 'Reset Settings',
    resetConfirm: 'Reset all settings to defaults? This will reload the page.',
    about: 'About',
    iosInstallTitle: 'To install on iOS:'
  },
  repeatModes: {
    none: 'Off',
    verse: 'Verse',
    surah: 'Surah'
  },
  tip: {
    both: 'Try landscape mode with auto-hide for a better reading experience',
    landscape: 'Try landscape mode for a wider, more immersive reading experience',
    autoHide: 'Enable auto-hide for a more immersive experience',
    enableAutoHide: 'Enable Auto-hide',
    enable: 'Enable',
    dismiss: 'Dismiss'
  },
  reading: {
    selectVerse: 'Select verse {n}',
    playFromHere: 'Play from here'
  },
  sr: {
    verseChange: 'Verse {verse} of {total}, {surah}',
    verseOf: 'Verse {current} of {total}, {surah}'
  },
  feature: {
    footnotesTitle: 'New: translation footnotes',
    footnotesBody:
      'Some translations (like Saheeh International) now show numbered notes. Tap a marker in the translation to read it. You can turn this off in Settings.',
    gotIt: 'Got it',
    dismiss: 'Dismiss'
  },
  errors: {
    notFoundAudio: 'Audio is unavailable for this reciter and surah.',
    notFoundText: 'This surah or translation could not be found.',
    network: 'Network error. Please check your connection and try again.',
    invalidAudio: 'The audio service is unavailable right now. Try another reciter.',
    invalidText: 'The text service is unavailable right now. Please try again.',
    generic: 'Failed to load surah. Please check your connection and try again.'
  },
  update: {
    available: 'Update available',
    ready: 'A new version is ready to install.',
    update: 'Update',
    later: 'Later',
    dismiss: 'Dismiss',
    applyingTitle: 'Updating…',
    applyingBody: 'Installing the new version. The page will reload in a moment.',
    applyingProgress: 'Please wait…'
  },
  install: {
    title: 'Install App',
    description: 'Add to your home screen for quick access.',
    install: 'Install',
    dismiss: 'Dismiss'
  },
  common: {
    noResults: 'No results found',
    of: 'of'
  },
  notFound: {
    title: 'Page not found',
    description: 'The page you are looking for does not exist.',
    home: 'Go to Quran Player'
  }
}
